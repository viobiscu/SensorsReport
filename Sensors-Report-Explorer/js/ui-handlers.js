 ...
 */
export function openFormQuantumLeapEntityView() {
    // Initialize TabManager if it doesn't exist yet
    if (!window.tabManager) {
        console.log('Initializing TabManager');
        window.tabManager = new TabManager('#displayArea');
    ...
    // Create tab title
    const tabTitle = 'Quantum Leap Entity View';
    ...
    const editorOptions = {
        ...
        pageSize: 10,
        // Configure visible toolbar buttons
        visibleToolbarButtons: {
            separator: false,
            attribute: false,
            relationship: false,
        ...
        // Add event handler for GET operation
        onOperation: async (editor, operation, entityId, type) => {
            if (operation !== 'GET') return;
            
            try {
                const client = new QuantumLeapClient();
                const data = await client.getEntityTimeSeries(entityId, type);
                console.log('[Debug] Received Quantum Leap entity data:', data);
                
                if (data) {
                    editor.setValue(JSON.stringify(data, null, 2));
                    appendToLogs(`Successfully fetched time series data for entity ${entityId}`);
                }
            } catch (error) {
