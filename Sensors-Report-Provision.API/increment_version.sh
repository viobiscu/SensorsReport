...
DOCKER_IMAGE="viobiscu/sensors-report-provision-api:$VERSION"
DOCKER_LATEST="viobiscu/sensors-report-provision-api:latest"
cd ..
docker build -f Sensors-Report-Provision.API/Dockerfile -t "$DOCKER_IMAGE" .
docker build -f Sensors-Report-Provision.API/Dockerfile -t "$DOCKER_LATEST" .
