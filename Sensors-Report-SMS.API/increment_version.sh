...
DOCKER_IMAGE="viobiscu/sensors-report-sms-api:$VERSION"
DOCKER_LATEST="viobiscu/sensors-report-sms-api:latest"
cd ..
docker build -f Sensors-Report-SMS.API/Dockerfile -t "$DOCKER_IMAGE" .
docker build -f Sensors-Report-SMS.API/Dockerfile -t "$DOCKER_LATEST" .
